...
public class AdapterOrders extends RecyclerView.Adapter<AdapterOrders.CustomViewHolder> {
    ...
    public AdapterOrders(ArrayList<Order> O, Context context, ItemClickListner itmClkListner)
    {
        OrderList = O;
    ...
    public class CustomViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener, View.OnCreateContextMenuListener {
        ...
        public void onCreateContextMenu(ContextMenu contextMenu, View view, ContextMenu.ContextMenuInfo contextMenuInfo) {
            contextMenu.add(this.getAdapterPosition(),666,0,"High Priority" );
            contextMenu.add(this.getAdapterPosition(),667,1,"Default Priority");
            contextMenu.add(this.getAdapterPosition(),668,2,"Low Priority" );
            contextMenu.add(this.getAdapterPosition(),669,3,"Hide Order");
            contextMenu.add(this.getAdapterPosition(),670,4,"Stop Tracking");
        }
    }
    ...
    @NonNull
    @Override
    public AdapterOrders.CustomViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View item = Linflater.inflate(R.layout.orderitem , parent , false);
        return new CustomViewHolder(item,gitemClickListner);
    }

    @Override
    public void onBindViewHolder(@NonNull AdapterOrders.CustomViewHolder holder, int position) {
        ...
        /*if (OrderList.get(position).getVisibility()==0)
            holder.icon1.setImageResource(R.drawable.hide);
        else if (OrderList.get(position).getVisibility()==-1)
